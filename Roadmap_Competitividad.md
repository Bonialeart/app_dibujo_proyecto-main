...
1. **Reglas de Perspectiva Pro (¡COMPLETADO!)**: Soporte para reglas magnéticas y guías de 1, 2 y 3 puntos de fuga que fuercen el trazo a converger de forma perfecta sobre el lienzo.
2. **Herramienta de Simetría en Tiempo Real (¡COMPLETADO!)**: Creación de espejos radiales, verticales, horizontales y de cuadrante (radial/espejo) para diseño de rostros, mandalas o concept art.
3. **Capas Vectoriales Inteligentes**: Capas especiales donde el lineart se almacena como curvas Bézier, permitiendo reescalar sin pixelación y usar el "Borrador Vectorial" (corta el trazo exactamente hasta la siguiente intersección).
4. **Herramienta de Licuado Acelerada (Liquify) (¡COMPLETADO!)**: Modos de *Empujar, Inflar, Pellizcar, Reconstruir y Torbellino* en tiempo real directamente sobre el lienzo usando aceleración por GPU.
5. **Generador de Tramas (Screentone) (¡COMPLETADO!)**: Herramientas dedicadas a la creación de manga/cómics, para aplicar patrones de puntos semitonos de forma no destructiva y configurable.
6. **Lazo Magnético y Selección por Gama de Colores**: Algoritmos avanzados de detección de bordes y canales de color para selecciones de área hiperprecisas.
...
8. **Deformación por Malla y Jaula (Mesh & Cage Warp) (¡NUEVO!)**: Herramienta avanzada para distorsionar y ajustar proporciones de ilustraciones o partes seleccionadas mediante una rejilla elástica interactiva.
9. **Motor de Pinceles 3D e Impasto Realista (¡NUEVO!)**: Motor de sombreado que genera relieve tridimensional en tiempo real, emulando la acumulación física de pintura al óleo o acrílica con mapas de alturas.
...
1. **Línea de Tiempo Dinámica (Timeline) (¡COMPLETADO!)**: Interfaz intuitiva en la parte inferior para gestionar fotogramas, celdas de animación, capas de animación y velocidad de reproducción (FPS).
2. **Papel Cebolla Avanzado (Onion Skinning) (¡COMPLETADO!)**: Visualización configurable de fotogramas anteriores (tonos rojos/cálidos) y posteriores (tonos verdes/fríos) con opacidad ajustable para guiar el movimiento.
...
6. **Soporte de Pista de Audio y Lip-Sync (¡NUEVO!)**: Importación de pistas de sonido (.mp3, .wav) directamente en el Timeline para sincronizar fotogramas y animación con voces de personajes o efectos sonoros.
...
3. **Globos de Texto Vectoriales (Speech Bubbles) (¡COMPLETADO!)**: Biblioteca de bocadillos de diálogo editables como curvas, con rabo dinámico apuntando al personaje y cajas de texto autoajustables.
...
7. **Filtros y Efectos de Manga (Halftoning Pro) (¡NUEVO!)**: Filtro rápido de conversión en escala de grises y tramado automatizado de capas para emular la impresión clásica de manga de manera no destructiva.
...
2. **Paneles Acoplables y Flotantes (Docking System) (¡COMPLETADO!)**: Capacidad total de arrastrar, soltar, apilar e integrar paneles a los lados izquierdo/derecho del lienzo para adaptar el espacio de trabajo.
3. **Ventana de Referencia Dinámica (Sub-view) (¡COMPLETADO!)**: Un panel flotante e independiente donde los artistas pueden cargar imágenes de referencia, extraer su paleta de colores y hacer zoom/pan sin alterar el lienzo activo.
4. **Ventana de Vista Previa (Navigator) (¡COMPLETADO!)**: Miniatura interactiva de todo el lienzo en tiempo real que permite orientarse y navegar rápidamente por composiciones gigantescas.
...
1. **Ajustes de Entrada de Lápiz y Tableta (Tablet API Selector) (¡COMPLETADO!)**:
...
5. **Personalización del Espacio Visual (Themes & Scaling) (¡COMPLETADO!)**:
...
2. **Máscaras de Recorte (Clipping Masks) (¡COMPLETADO!)**: Capacidad de anclar una capa al contorno y transparencia de la capa inferior.
...
6. **Curvas de Presión por Pincel (¡COMPLETADO!)**: Sensibilidad a la presión e inclinación ajustable por cada herramienta individualmente.
7. **Importador de Recursos (Imágenes y Pinceles) (¡COMPLETADO!) (¡NUEVO!)**: Carga de imágenes directamente como nuevas capas y soporte para la importación nativa de pinceles profesionales de Photoshop (.ABR) en el motor de dibujo.
...
2. **Grabación de Timelapse en Segundo Plano (¡COMPLETADO!)**: Captura y almacenamiento asíncrono y ultra-rápido de fotogramas del lienzo en segundo plano sin interrumpir el trazo del artista.
...
6. **Exportación Avanzada de Timelapse (¡NUEVO!)**: Compresión y compilación asíncrona de los fotogramas guardados en disco a un vídeo MP4 (H.264), con opciones de orientación vertical (9:16) para TikTok/Reels, velocidad ajustable y marcas de agua.
7. **Sincronización en la Nube y Control de Versiones (¡NUEVO!)**: Guardado y respaldo en segundo plano compatible con servicios como Google Drive o Dropbox, con historial de versiones anteriores para prevenir pérdidas accidentales de datos.
